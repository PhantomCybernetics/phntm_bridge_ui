...
export class GraphMenu {
	constructor(ui) {
		...
		this.graph_display_el.on("mousemove touchstart", (ev) => {
			...
			if (ev.targetTouches)
				that.pointer_coords = [
					ev.targetTouches[0].pageX - off.left,
					ev.targetTouches[0].pageY - off.top,
				];
			else that.pointer_coords = [ev.pageX - off.left, ev.pageY - off.top];
		});
	...
	setDimensions(available_w, h) {
		...
		if (this.is_narrow) {
			...
			if (this.width_svg < 0) this.width_svg = 0;
			...
			if (this.svg) {
				...
				this.svg.attr("width", this.width_svg).attr("height", this.height);
	...
	setLinkTooltip(link, el, topic = false) {
		...
		tooltip +=
			...
			"<br>\n" +
			"Lifespan: " +
			(link.qos.lifespan < 0 ? "INFINITE" : link.qos.lifespan / NS_TO_SEC + "s") +
			"<br>\n" +
			"Deadline: " +
			(link.qos.deadline < 0 ? "INFINITE" : link.qos.deadline / NS_TO_SEC + "s") +
			"<br>\n" +
			"History: " +
			history +
			"<br>\n" +
	...
	update(nodes) {
		...
		this.node_ids.forEach((id_node) => {
			...
			if (nodes[id_node].publishers) {
				...
				topic_ids.forEach((id_topic) => {
					...
					for (let i = 0; i < qos.length; i++) {
						// multiple publishes of the same topic on one node
			...
			if (nodes[id_node].subscribers) {
				...
				topic_ids.forEach((id_topic) => {
					...
					for (let i = 0; i < qos.length; i++) {
						// multiple subscribers to the same topic on one node
		...
		this.topic_ids.forEach((topic) => {
			...
			chb.on("change", (e) => {
				if (chb.hasClass("disabled")) return;
		...
		this.links.forEach((link) => {
			...
			link.path = this.svg
				...
				.style(
					"stroke",
					link.qos_error || link.qos_warning
						? this.color_err
						: link.group == 1
							? this.color_write
							: this.color_read,
			...
			link.path.on("mouseenter click", function () {
				if (
					...
					link.path.attr("class").indexOf("dimmed") > -1
				)
					return;
				...
				link.path.attr(
					link.group == 1 ? "marker-end" : "marker-start",
					link.group == 1
						? "url(#marker-hover-write)"
						: "url(#marker-hover-read)",
				);
	...
	getLinkPath(link) {
		...
		let y_node =
			-this.node_container_el.scrollTop() + 10 + n.offset + link.node_conn_no * 5;
		let y_topic =
			-this.topic_scrolling_el.scrollTop() + 10 + t.offset + link.topic_conn_no * 5;
	...
	nodeFocusToggle(id_node) {
		...
		if (focused) this.focused_id_node = id_node;
		else if (this.focused_id_node == id_node) this.focused_id_node = null;
	...
	topicFocusToggle(topic) {
		...
		if (focused) this.focused_topic = topic;
		else if (this.focused_topic == topic) this.focused_topic = null;
	...
	hoverNode(id_node, state) {
		...
		if (state) this.hovered_id_node = id_node;
		else if (!state && this.hovered_id_node == id_node) this.hovered_id_node = null;
	...
	hoverTopic(topic, state) {
		...
		if (state) this.hovered_topic = topic;
		else if (!state && this.hovered_topic == topic) this.hovered_topic = null;
	...
	updateHighlights() {
		...
		this.node_ids.forEach((id_node) => {
			...
			if (
				(!something_focused && (n.highlighted || n.highlighted_connection)) ||
				(something_focused && (n.focused || n.focused_connection))
			) {
		...
		this.links.forEach((link) => {
			...
			if (
				(!something_focused && link.highlighted_connection) ||
				(something_focused && link.focused_connection)
			) {
		...
		this.topic_ids.forEach((topic) => {
			...
			if (
				(something_focused && (t.focused || t.focused_connection)) ||
				(!something_focused && (t.highlighted || t.highlighted_connection))
			) {
