...
export class SystemLoadWidget {
    ...
    onData(decoded) {
        ...
        if (!this.initiated || num_lines != this.num_lines) {
            ...
            let line_h = 100.0 / this.num_lines;
            let line_h_px = h / this.num_lines;
            if (line_h_px < 20)
                this.el.addClass('thin');
            else
        ...
        for (let i = 0; i < decoded.cpu.length; i++) {
            ...
            let nice_w = Math.ceil(w/100.0 * decoded.cpu[i].nice_percent / 7) * 7;
            this.cpu_bars[i]['nice'].css('width', nice_w+'px');
            let user_w = Math.ceil(w/100.0 * decoded.cpu[i].user_percent / 7) * 7;
            this.cpu_bars[i]['user'].css('width', user_w+'px');
            let system_w = Math.ceil(w/100.0 * decoded.cpu[i].system_percent / 7) * 7;
