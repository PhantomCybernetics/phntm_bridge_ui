...
class TrackballControls extends Controls {
	...
	constructor( object, domElement = null ) {
		...
		this._onLostPointerCapture = onLostPointerCapture.bind( this );
	...
	connect( element ) {
		...
		// [ 'pointerdown', 'pointerup', 'pointermove', 'pointercancel',  'wheel', 'contextmenu'].forEach(type => 
		// 	this.domElement.addEventListener(type, (e) => {
		// 		if (type != 'mousemove' && type != 'pointermove')
		// 			console.warn(`${type}:`, e.clientX, e.clientY);
		// 		//else
		// 		//	console.log(`${type}:`, e.clientX, e.clientY);
	...
	update() {
		...
		if (this.fixHorizon)
			...
			this.object.up.set(0,1,0).applyQuaternion(this.object.quaternion);
	...
	_panCamera() {
		...
		if ( _mouseChange.lengthSq() ) {
			...
			_pan.add( _objectUp.set(0,1,0).applyQuaternion(this.object.quaternion).setLength( _mouseChange.y ) );

			this.object.position.add( _pan );
			this.lookTarget.add( _pan );
	...
	_rotateCamera() {
		...
		if ( angle ) {
			...
			_eyeDirection.copy( this.fixHorizon ? new Vector3(this._eye.x, 0, this._eye.z) : this._eye).normalize();
			const up = this.fixHorizon ? new Vector3(0,1,0) : this.object.up;
			_objectUpDirection.copy(up).normalize();
			...
			_axis.crossVectors( _moveDirection, _eyeDirection ).normalize();
