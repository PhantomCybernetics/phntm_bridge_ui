export class InputDriver {
	...
	getConfig() {
		// to save as cookie, on robot, etc
		let cfg = {
	...
	checkSaved() {
		...
		if (this.saved_state.msg_type != this.msg_type) return false;
		if (this.saved_state.output_topic != this.output_topic) return false;
	...
	handleErrorMessage() {
		if (this.error_message && this.error_label) {
			this.error_label.html(this.error_message).css("display", "block");
			if (this.inp_topic) this.inp_topic.addClass("error");
		} else if (!this.error_message && this.error_label) {
			this.error_label.empty().css("display", "none");
			if (this.inp_topic) this.inp_topic.removeClass("error");
		}
	}
	...
	displayOutput(el, transmitting) {
		el.html(
			"Message: <b>" +
				...
				this.output_topic +
				"</b> " +
				(transmitting ? "(transmitting)" : "(not transmitting)") +
				"<br><br>" +
